...
export default function ProjectCard() {
	...
	return (
		<Link href='/projects/project-id'>
			<a className='group cursor-pointer flex flex-col gap-2 rounded-2xl bg-white/10 p-4 backdrop-blur-sm border border-white/25 overflow-hidden'>
				<ul className='flex text-xs gap-1.5 font-medium capitalize text-black/50'>
					{stack.map((item, idx) => (
						<li key={idx}>#{item}</li>
					))}
				</ul>
				...
				<div className='space-y-2'>
					...
					<h3 className='text-sm font-semibold text-black leading-relaxed tracking-wide line-clamp-2'>
